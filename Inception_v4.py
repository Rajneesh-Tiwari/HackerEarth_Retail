...
def perspective(img):
    ...
    shrink_ratio1 = np.random.randint(low=85, high=115, dtype=int) / 100
    shrink_ratio2 = np.random.randint(low=85, high=115, dtype=int) / 100
...
base_model = inception_v4.create_model(weights='imagenet', include_top=False,dropout_prob=0.,num_classes=None)
...
df_test.to_csv('submission_inceptionvv4_tta_modifiedaugmentation.csv', index=False)
