...
callbacks = [ModelCheckpoint('Retail_V1.hdf5', monitor='val_loss', save_best_only=True, verbose=2, save_weights_only=False),
             ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=4, verbose=1, mode='auto', epsilon=0.0001, cooldown=0, min_lr=0.0000001),
...
# Load our model
model = SqueezeNet(include_top=False, weights=None,
               input_tensor=None, input_shape=None,
               classes=1000,pooling='avg')
x = model.output
x = Dense(256, activation='relu')(x)
x = Dense(64, activation='relu')(x)
output = Dense(len(labels), activation='softmax')(x)
model_sqnet = Model(inputs=model.inputs, outputs=output)

optimizer = Adam(0.001, decay=0.0003)
model_sqnet.compile(loss='categorical_crossentropy', optimizer=optimizer, metrics=['accuracy'])
model_sqnet.summary()
...
# Start Fine-tuning
model_sqnet.fit_generator(generator=batch_generator_train(list(zip(X_train, Y_train)), IMG_SIZE, BATCH),
...
probs = model_sqnet.predict_generator(predict_generator(x_test,IMG_SIZE,batch_size_test), steps=np.ceil(len(x_test)/batch_size_test),verbose=1)
...
df_test.to_csv('submission_.csv', index=False)
